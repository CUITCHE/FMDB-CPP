...
- (void)testOpenWithVFS
{
    ...
    db.openWithFlags(SQLITE_OPEN_READWRITE | SQLITE_OPEN_CREATE, "MyCustomVFS");
...
- (void)testSelectByColumnName
{
    ...
    while (rs->next()) {
        rs->intForColumn("c");
        XCTAssertTrue(rs->stringForColumn("b"), @"Should have non-nil string for 'b'");
        XCTAssertTrue(rs->stringForColumn("a"), @"Should have non-nil string for 'a'");
        XCTAssertTrue(rs->stringForColumn("rowid"), @"Should have non-nil string for 'rowid'");
        XCTAssertTrue(rs->dateForColumn("d"), @"Should have non-nil date for 'd'");
        rs->doubleForColumn("d");
        rs->doubleForColumn("e");

        XCTAssertEqual(rs->columnNameForIndex(0), "rowid",  @"Wrong column name for result set column number");
        XCTAssertEqual(rs->columnNameForIndex(1), "a",      @"Wrong column name for result set column number");
    }
...
- (void)testSelectWithIndexedAndKeyedSubscript
{
    ...
    while (rs->next()) {
        XCTAssertEqual((*rs)[0], (*rs)["rowid"], @"Column zero should be equal to 'rowid'");
        XCTAssertEqual((*rs)[1], (*rs)["a"], @"Column 1 should be equal to 'a'");
        XCTAssertEqual((*rs)[2], (*rs)["b"], @"Column 2 should be equal to 'b'");
        XCTAssertEqual((*rs)[3], (*rs)["c"], @"Column 3 should be equal to 'c'");
    }
...
- (void)testBusyRetryTimeout
{
    ...
    self.db->setMaxBusyRetryTimeInterval(TimeInterval(2));
...
- (void)testCaseSensitiveResultDictionary
{
    ...
    self.db->executeUpdate("insert into cs (aRowName, bRowName) values (?, ?)", true ,"hello");
    ...
    while (rs->next()) {
        ...
        XCTAssertTrue(d["aRowName"].isNull() == false, @"aRowName should be non-nil");
        XCTAssertFalse(d["arowname"].isNull() == false, @"arowname should be nil");
        XCTAssertTrue(d["bRowName"].isNull() == false, @"bRowName should be non-nil");
        XCTAssertFalse(d["browname"].isNull() == false, @"browname should be nil");
    }
...
- (void)testBoolInsert
{
    ...
    self.db->executeUpdate("insert into btest (aRowName) values (?)", Variant(12).toBool());
...
- (void)testNSNullInsertion
{
    ...
    while (rs->next()) {
        ...
        XCTAssertTrue(rs->stringForColumnIndex(1));
    }
...
- (void)testNullDates
{
    ...
    while (rs->next()) {
        ...
        auto b = rs->dateForColumnIndex(1);
        auto c = rs->dateForColumnIndex(2);

        XCTAssertFalse(rs->dateForColumnIndex(0));
        XCTAssertTrue(c, @"zero date shouldn't be nil");
...
- (VariantData)NSDataToVarintData:(NSData *)data
{
    const unsigned char *d = (const unsigned char *)data.bytes;
    return VariantData(d, d + data.length);
}

- (void)testLotsOfNULLs
{// SaverBackground.png compass.icns
    NSData *safariCompass = [NSData dataWithContentsOfFile:@"/Applications/Safari.app/Contents/Resources/SaverBackground.png"];
    ...
    while (rs->next()) {
        ...
        if (i == 12) {
            ...
            auto d = rs->dataForColumn("d");
            NSData *_d = [NSData dataWithBytes:d->data() length:d->size()];
            XCTAssertEqualObjects(_d, safariCompass);
            XCTAssertFalse(rs->dataForColumn("notthere"));
            XCTAssertFalse(rs->stringForColumnIndex(-2), @"Negative columns should return nil results");
            ...
            XCTAssertFalse(rs->dataForColumn("d"));
        }
    }
...
- (void)testColumnNamesContainingPeriods
{
    ...
    XCTAssertEqual(*rs->stringForColumn("t4.a"), "one");
    XCTAssertEqual(*rs->stringForColumn("b"), "two");
...
- (void)testDatabaseAttach
{
    ...
    FMDatabase dbB("/tmp/attachme.db");
...
- (void)testPragmaDatabaseList
{
    ...
    while (rs->next()) {
        counter++;
        XCTAssertEqualObjects(@(rs->stringForColumn("file")->c_str()), self.databasePath);
    }
...
- (void)testColumnNameMap
{
    ...
    XCTAssertEqual(d.size(), (NSUInteger)4);

    XCTAssertEqualObjects(@(d["a"]), @0);
    XCTAssertEqualObjects(@(d["b"]), @1);
    XCTAssertEqualObjects(@(d["c"]), @2);
    XCTAssertEqualObjects(@(d["d"]), @3);
...
- (void)testCustomFunction
{
    ...
    self.db->makeFunctionNamed("StringStartsWithH", 1, [=](void *context, int aargc, void **aargv) {
        if (sqlite3_value_type((sqlite3_value *)aargv[0]) == SQLITE_TEXT) {

            @autoreleasepool {

                const char *c = (const char *)sqlite3_value_text((sqlite3_value *)aargv[0]);
                NSString *s = [NSString stringWithUTF8String:c];
                sqlite3_result_int((sqlite3_context *)context, [s hasPrefix:@"h"]);
            }
        }
        else {
            XCTFail(@"Unknown format for StringStartsWithH (%d)", sqlite3_value_type((sqlite3_value *)aargv[0]));
...
- (void)testExecuteStatements
{
    ...
    success = self.db->executeStatements(sql.UTF8String);
    ...
    success = self.db->executeStatements(sql.UTF8String, [=](unordered_map<string, Variant> &result) -> int {
        NSInteger count = result["count"].toLongLong();
        XCTAssertEqual(count, 1, @"expected one record for dictionary %s", Variant(result).description().c_str());
        return 0;
    });
...
- (void)testCharAndBoolTypes
{
    ...
    XCTAssertEqualObjects(@(rs->objectForColumnName("a").toBool()), @YES);

    XCTAssertEqual(rs->boolForColumn("b"), false);
    XCTAssertEqualObjects(@(rs->objectForColumnName("b").toBool()), @NO);

    XCTAssertEqual(rs->intForColumn("c"), 'x');
    XCTAssertEqualObjects(@(rs->objectForColumnName("c").toInt()), @('x'));
...
{
    NSString *version = @(FMDatabase::sqliteLibVersion().c_str());
...
- (void)testOpenZeroLengthPath
{
    ...
    XCTAssert(db.executeUpdate("create table foo (bar text)"), "create failed");
    NSString *value = @"baz";
    XCTAssert(db.executeUpdate("insert into foo (bar) values (?)","baz"), @"insert failed");
    NSString *retrievedValue = @(db.stringForQuery("select bar from foo")->c_str());
    XCTAssert([value compare:retrievedValue] == NSOrderedSame, @"values didn't match");
}
...
- (void)testChangingMaxBusyRetryTimeInterval
{
    ...
    db.setMaxBusyRetryTimeInterval(TimeInterval(updatedInterval));
...
- (void)testGoodConnection
{
    FMDatabase db("");
    XCTAssert(db.open(), @"open failed");
    XCTAssert(db.isGoodConnection(), @"no good connection");
}
...
- (void)testBadConnection
{
    ...
    XCTAssertFalse(db.isGoodConnection(), @"no good connection");
}
...
- (void)testLastRowId
{
    ...
    XCTAssert(db.executeUpdate("insert into foo (bar) values (?)","qux"), @"insert failed");
...
- (void)testChanges
{
    ...
    XCTAssert(db.executeUpdate("insert into foo (bar) values (?)","baz") , @"insert failed");
    XCTAssert(db.executeUpdate("insert into foo (bar) values (?)" ,"qux"), @"insert failed");
    XCTAssert(db.executeUpdate("update foo set bar = ?" ,"xxx"), @"insert failed");
...
- (void)testBind {
    ...
    XCTAssert(db.executeUpdate("insert into foo (a) values (?)" ,insertedValue.charValue), @"insert failed");
    ...
    XCTAssert(db.executeUpdate("insert into foo (a) values (?)" ,insertedValue.shortValue), @"insert failed");
    retrievedValue = @(db.intForQuery("select a from foo where id = ?", db.lastInsertRowId()));
    XCTAssertEqualObjects(insertedValue, retrievedValue, @"values don't match");
    ...
    XCTAssert(db.executeUpdate("insert into foo (a) values (?)" , insertedValue.intValue), @"insert failed");
    retrievedValue = @(db.intForQuery("select a from foo where id = ?", db.lastInsertRowId()));
    XCTAssertEqualObjects(insertedValue, retrievedValue, @"values don't match");

    insertedValue = [NSNumber numberWithUnsignedInt:55];
    XCTAssert(db.executeUpdate("insert into foo (a) values (?)" ,insertedValue.unsignedIntValue), @"insert failed");
    retrievedValue = @(db.intForQuery("select a from foo where id = ?", db.lastInsertRowId()));
    XCTAssertEqualObjects(insertedValue, retrievedValue, @"values don't match");

    insertedValue = [NSNumber numberWithLong:56];
    XCTAssert(db.executeUpdate("insert into foo (a) values (?)" , insertedValue.longLongValue), @"insert failed");
    ...
    XCTAssert(db.executeUpdate("insert into foo (a) values (?)" , insertedValue.unsignedLongLongValue), @"insert failed");
    retrievedValue = @(db.longForQuery("select a from foo where id = ?", db.lastInsertRowId()));
    XCTAssertEqualObjects(insertedValue, retrievedValue, @"values don't match");

    insertedValue = [NSNumber numberWithFloat:58];
    XCTAssert(db.executeUpdate("insert into foo (a) values (?)" , insertedValue.floatValue), @"insert failed");
    retrievedValue = @(db.doubleForQuery("select a from foo where id = ?", db.lastInsertRowId()));
    XCTAssertEqualObjects(insertedValue, retrievedValue, @"values don't match");

    insertedValue = [NSNumber numberWithDouble:59];
    XCTAssert(db.executeUpdate("insert into foo (a) values (?)" , insertedValue.doubleValue), @"insert failed");
    retrievedValue = @(db.doubleForQuery("select a from foo where id = ?", db.lastInsertRowId()));
    XCTAssertEqualObjects(insertedValue, retrievedValue, @"values don't match");

    insertedValue = @TRUE;
    XCTAssert(db.executeUpdate("insert into foo (a) values (?)" , insertedValue.boolValue), @"insert failed");
