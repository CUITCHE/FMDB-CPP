...
#endif
    if (!success) {
        _assert(false, "Could not create database queue for path %s", path.c_str());
        return;
    }
...
{
    _assert(this_thread::get_id() != _packet->_thread->get_id(), "Don't call any methods in database queue!");
}
